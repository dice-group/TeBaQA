...
public class ArffGeneratorTriples {
    public void trainModel(AbstractClassifier classifier,Instances trainingSet){
    ...
    public static Analyzer getAnayzer(Attribute classAttribute){
    ...
    ArffGeneratorTriples(Instances trainingSet,String name,  Boolean evaluateWekaAlgorithms) {
        List<Attribute> attributes = new ArrayList<>();
        ...
        trainingSet.addAll(trainQuestions);*/
        ...
        try {
            File file = new File("./src/main/resources/Train_"+name+".arff");
            file.createNewFile();
            writeSetToArffFile(trainingSet, file.getPath());
        } catch (IOException e) {
            e.printStackTrace();
       ...
       /* try {
        ...
        try {
            trainingSet.setClassIndex(trainingSet.numAttributes() - 1);
            classifier.buildClassifier(trainingSet);
            SerializationHelper.write("./src/main/resources/question_classification_"+name+".model", classifier);
        } catch (Exception e) {
            e.printStackTrace();
        }
