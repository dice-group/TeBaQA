...
public class SPARQLUtilities {
    ...
    private static final Pattern SPLIT_TRIPLE_PATTERN = Pattern.compile("[^\\s\"']+|\"([^\"]*)\"|'([^']*)'");
    private static final Logger log = LogManager.getLogger(SPARQLUtilities.class);
...
//            QueryExecution qe = QueryExecutionFactory.sparqlService("http://dbpedia.org/sparql", query);
    ...
    private static boolean isCountQuery(String query) {
        return TextUtils.trim(query.toLowerCase()).startsWith("select (count");
    }

    static String transformCountToStarQuery(String s) {
        s = TextUtils.trim(s);
        return "SELECT * " + s.substring(s.toLowerCase().indexOf(" where"));
    ...
    static String replaceWithWildcard(String sparlQuery) {
        ...
        sparlQuery = "SELECT * " + sparlQuery.substring(endPosition);
        return sparlQuery;
    }
    ...
    public static String resolveNamespaces(String query) {
        if (!query.toLowerCase().contains("prefix")) {
            ...
            if (nsPrefixMap.isEmpty()) {
                ...
                final String[] queryWithoutPrefix = {query.substring(startOfQuery)};
